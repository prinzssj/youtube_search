        ...
        callbacks: {
            ...
            video_playback: function (data) {
                ...
                if (data.video.items.length > 0) {
					...
					var ytdl_config = {
						ytdl_options: '--cookies "cookies.txt" --no-check-certificate --format best'
					};				
                    ...
                    */
                    switch (youtube.config.plugin.ytdl_action) {
						case 1: // Download
							if (queue) {
								try {
									if (media.ytdl(videoId, false, ytdl_config.ytdl_options)) {
										engine.log("Download & append to queue: " + videoId);
										if (media.enqueueYt(videoId)) {
											engine.log("Successfully enqueued: " + videoId);
										}
									} else {
										engine.log("Cannot download: " + videoId);
									}
								} catch(e) {
									engine.log("Error en download: " + e);
								}
							} else {
								try {
									if (media.ytdl(videoId, (queue ? false : true), ytdl_config.command_options)) {
										engine.log("Download & play: " + videoId);
									} else {
										engine.log("Can't download: " + videoId);
									}
								} catch(e) {
									engine.log("Download error: " + e);
								}
							}
							return true;
                        case 2: // Stream
