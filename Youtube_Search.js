                ...
                if (data.video.items.length > 0) {
                    ...
                    */
                    switch (youtube.config.plugin.ytdl_action) {
                        case 1: // Download
							if (queue) {
								if (media.ytdl(videoId, false, '--cookies "/opt/sinusbot/scripts/cookies.txt"') && media.enqueueYt(videoId)) {
                        ...
                        case 2: // Stream
							if (queue) {
								if (media.enqueueYt(videoId, '--cookies "/opt/sinusbot/scripts/cookies.txt"')) {
									engine.log("Append to queue: " + videoId);
								}
							} else {
								if (media.yt(videoId, '--cookies "/opt/sinusbot/scripts/cookies.txt"')) {
