                ...
                if (data.video.items.length > 0) {
                    ...
                    */
                    switch (youtube.config.plugin.ytdl_action) {
                        case 1: // Download
							const options = {
								cookies: "cookies.txt" // Añadir el parámetro de cookies
							};

							if (queue) {
								if (media.ytdl(videoId, options) && media.enqueueYt(videoId)) {
									engine.log("Download & append to queue: " + videoId);
								} else {
									engine.log("Cannot enqueue: " + videoId);
								}
							} else {
								if (media.ytdl(videoId, { ...options, queue: false })) { // Asegúrate de que el segundo argumento sea correcto
